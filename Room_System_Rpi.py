...
alarm_time_hour = 5 # 5 am default
alarm_time_minute = 0
...
while True:
    ...
    if not Alarm_Lights_Flash and not Alarm_Lights and 
    ...
    if Alarm_Lights: gpio.output(Lights_Pin, gpio.HIGH)
    else: gpio.output(Lights_Pin, gpio.LOW)
    if Alarm_Lights_Flash: gpio.output(Flash_Pin, gpio.HIGH)
    else: gpio.output(Lights_Pin, gpio.LOW)
